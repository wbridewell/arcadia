(ns arcadia.models.stroop-gratton
...
(defn component-setup []
  (model/setup
   ...
   (model/add response-stm {:threshold 11}) ;13 for SOA, 11 for gratton 
...
(defn run-experiment
  ...
  ((resolve 'arcadia.experiments/startup-experiment)
   ...
   :save-data? output-file
   :data-parameters {:output-file output-file}
   :env-parameter-ranges [[:weights [{:congruent 1 :incongruent 1}]]
                          [:color-onset [0]]]))
