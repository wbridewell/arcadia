(ns
 ...
 arcadia.simulator.environment.video-player
  ...
  (:require
   ...
   [arcadia.simulator.environment.core :refer [Environment render]]
...
(defrecord VideoPlayer [camera file-path]
  ...
  (info [env]
    {:dimensions [:height :width]
     ...
     :render-modes ["human" "buffered-image" "opencv-matrix" "numpy-array" "gpu-matrix"]
     :file-path file-path
     :actions nil})
...
(defn configure [env-args]
  (let [file-path (or (:video-path env-args) (:file-path env-args))]
    (if (nil? file-path)
      (throw (NullPointerException. "Environmental argument :video-path is nil."))
      (let [camera (theater/create-theater-camera (:video-path env-args) env-args)]
        (->VideoPlayer
         camera file-path)))))