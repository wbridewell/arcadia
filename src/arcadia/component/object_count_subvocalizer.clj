...
(defn subvocalize-count [value duration cyclecount source]
  (println "LEXEME-VALUE =" value "; cyclecount = " (str cyclecount))
  {:name "vocalize"
...
(defn update-cycle-count [vocalize-il source]
  (println (str "LEXEME-VALUE = " (:lexeme (:arguments vocalize-il)) "; cycle = " (:cycle (:arguments vocalize-il))))
  ...
  (receive-focus
   ...
   (let [number (filter #(and (= (:name %) "number")
     ...
     (cond
       ...
       (= (:name focus) "vocalize")
       (reset! (:buffer component) (update-cycle-count focus component))
       ...
       (reset! (:buffer component) (subvocalize-count (:value (:arguments number-comparison))
                                                      (calculate-duration (:value (:arguments number-comparison))) 0 component))
