...
(defn report-numerosity [number source]
  {:name "number-report"
...
(defn report-numerosity-subset [number description source]
  {:name "number-report"
...
(defn- pattern-recognition-result [result source]
    (let [results (:results (:arguments result))
         ...
         (if (empty? numbers) nil
             (report-numerosity (rand-nth numbers) source))))
  ...
  (receive-focus
   ...
   (let [ans (first (filter #(= (:name %) "number-sense") content))
       ...
       (cond
         ...
         (reset! (:buffer component) (report-numerosity (guess explicit-count ans) component))

         (and mask-present? ans vstm-enum)
         (reset! (:buffer component) (report-numerosity (guess vstm-enum ans) component))

         (and mask-present? (= (:name focus) "vstm-enumeration"))
         (reset! (:buffer component) (report-numerosity (:count (:arguments vstm-enum)) component))

         (and sweep-complete? explicit-count)
         (reset! (:buffer component) (report-numerosity-subset (:value (:arguments explicit-count)) descriptors component))

         pattern-result
         (reset! (:buffer component) (pattern-recognition-result pattern-result component))
