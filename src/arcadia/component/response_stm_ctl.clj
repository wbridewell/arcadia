(ns arcadia.component.response-stm-ctl
...
(defn- task-inhibition [source response-type inhibit-conflict? control]

  ; (println "SOURCE: " source ",  RESPONSE-TYPE: " response-type)
  ; (println "NOT= " (not= source response-type))
  ; (println "INHIBIT-CONFLICT? " inhibit-conflict?)
  ; (println "CONTROL-IN: " control)
  ; (println "CONTROL-OUT: "(if (not= response-type source) control 0))

  ;(if (not= response-type source) control 0))
  (cond (and (not= response-type source) inhibit-conflict?) control
        (not= response-type source) 1
        :else 0))

(defn- semantic-support [x semantic-data response-type inhibit-conflict? threshold control]

  ;(display/env "Semantic Support")
  ...
  (reduce -
          ...
          (map #(/ (* (semantic-distance x %) (:strength %)
                      (task-inhibition (:source %) response-type inhibit-conflict? control))
                   threshold)
               semantic-data)))

(defn- accumulate-support [semantic-elements response-type inhibit-conflict? threshold control]
    ...
    (if (empty? curr)
      ...
      (recur sdata (rest curr)
             (update m (:value (first curr)) (fnil + 0)
                     (semantic-support (first curr) sdata response-type inhibit-conflict? threshold control))))))
...
(defn start [& {:as args}]
  (let [p (merge-parameters args)]
    (->ResponseSTMCTL (atom nil) (atom {}) (atom nil) (atom false) (atom (:threshold p)) (atom 1))))
