...
(defn- make-fixation [segment focus-object]
  {:name "fixation"
...
(defn- at-loc
  ...
  [segment location]
  (geo/intersect? (:region segment) (-> location :arguments :region)))

(defrecord ProximityHighlighter [buffer max-dist]
  Component
  (receive-focus
         ...
         ;; the closest segment to the focus NOT already in VSTM
          closest (when focus-object
                    (some->> content
                             ...
                             (sort-by #(geo/distance (:region %) focus-region))
                             first))]
      (when (and focus-object closest
                 (some-> focus-object :arguments :threat-change neg?)
                 (<= (geo/distance (:region closest) focus-region)
                     (:max-dist component)))
        (reset! (:buffer component) (make-fixation closest focus-object)))))
